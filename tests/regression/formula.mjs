#!/usr/bin/env node
// ============================================================================
// formula.mjs — MathML→OMML 转换器字节级回归测试
// ----------------------------------------------------------------------------
// 用法: node tests/regression/formula.mjs [--verbose]
// 对比 packages/model/mathml2omml.js 的输出与微软官方 MML2OMML.XSL 的固化参考
// （tests/fixtures/formula/omml-ai/），逐字符一致才算通过。
//
// 语料: tests/fixtures/formula/formulas.txt（204 个 LaTeX 用例）
//       mml/     KaTeX 输出（tests/tools/dump-formula-mml.mjs 生成，不入库）
//       omml-ai/ 官方 XSLT 固化参考（早年经 Office MML2OMML.XSL 批量生成）
//
// 重新生成 mml（改语料后）: npm run test:fixtures
...
import { mathmlToOmml } from "../../packages/model/mathml2omml.js";
...
if (!existsSync(MML_DIR)) {
  console.error("✗ 缺少 tests/fixtures/formula/mml/（KaTeX 中间产物，不入库）。先运行：npm run test:fixtures");
  process.exit(1);
}
