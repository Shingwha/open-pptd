...
`tests/regression/formula.mjs` 对比 `packages/model/mathml2omml.js` 输出与微软官方
`MML2OMML.XSL` 固化参考（`tests/fixtures/formula/omml-ai/`）。
...
Office/root/Office16/MML2OMML.XSL`）批量生成后固化入库（生成脚本为一次性工具，
已移除；参考输出即 PowerPoint 导出公式的真实行为）。KaTeX MathML（`mml/`）由仓库内
`packages/model/vendor/katex.mjs` 生成，与编辑器运行时同源。
