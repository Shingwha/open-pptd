...
import { THEME_PRESETS } from "../editor/core/theme.js";
import { buildPptx, magicMatches } from "../editor/writer/pptx.js";
import { collectFontSpecs } from "../editor/writer/font.js";
...
/** 字体加载器（Node）：file 项按 deck 目录相对路径读入 fontFiles；url 项交给 writer fetch。 */
async function createFontFiles(deck, deckDir) {
  const fontFiles = {};
  for (const spec of collectFontSpecs(deck)) {
    if (!spec.file || fontFiles[spec.family]) continue;
    try {
      fontFiles[spec.family] = new Uint8Array(readFileSync(join(deckDir, spec.file)));
    } catch (err) {
      console.warn(`[export] 字体文件缺失，已跳过: ${spec.file}`);
    }
  }
  return fontFiles;
}
 ...
 * @param {object} opts
 * @param {string} opts.manifest .pptd 文件路径
 * @param {string} [opts.outPath] 输出路径（缺省 <deck 目录>/<标题>.pptx）
 * @param {string} [opts.theme] 覆盖主题（THEME_PRESETS 的 key）
 * @param {boolean} [opts.embedFonts=true] 是否嵌入字体（false = 仅声明不嵌入）
 * @returns {Promise<{bytes: Uint8Array, outPath: string}>}
 */
export async function exportDeck({ manifest, outPath = null, theme = null, embedFonts = true }) {
  ...
  if (theme) {
    if (!THEME_PRESETS[theme]) {
      throw new Error(`未知主题「${theme}」（可用: ${Object.keys(THEME_PRESETS).join(" / ")}）`);
    }
    deck.theme = THEME_PRESETS[theme];
  }
  const bytes = await buildPptx(deck, {
    loadImage: createLoadImage(deckDir),
    fontFiles: embedFonts ? await createFontFiles(deck, deckDir) : null,
    embedFonts,
  });
